...
class SkeletonEmbedding:
    ...
    def distance_to_embedding(self, embedding: np.ndarray, method: str = 'mahalanobis') -> float:
        ...
        """
        if method == 'euclidean':
            return np.linalg.norm(embedding - self.mean)
        
        elif method == 'cosine':
            return 1 - np.dot(embedding, self.mean) / (
...
class SkeletonBasedDetector(nn.Module):
    ...
    def forward(self, x: torch.Tensor, prompts: Optional[torch.Tensor] = None) -> Dict[str, torch.Tensor]:
        ...
        frame_features = self.base_detector.extract_frame_features(x_flat)
        ...
        elif self.enable_multitask and prompts is None:
            raise ValueError("Multi-task learning is enabled but prompts were not provided")
    ...
    def save_skeletons(self, save_dir: str):
        ...
        if self.ai_skeleton:
            self.ai_skeleton.save(save_path / "ai_skeleton.pkl")
        
        if self.real_skeleton:
            self.real_skeleton.save(save_path / "real_skeleton.pkl")
    ...
    def load_skeletons(self, save_dir: str):
        ...
        if ai_skeleton_path.exists():
            self.ai_skeleton = SkeletonEmbedding.load(ai_skeleton_path)
        
        if real_skeleton_path.exists():
            self.real_skeleton = SkeletonEmbedding.load(real_skeleton_path)
