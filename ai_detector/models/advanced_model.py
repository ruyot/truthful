...
class TemporalCNNAggregator(nn.Module):
    """
    Aggregates predictions from multiple frames using 1D convolutions.
    """
    
    def __init__(self, feature_dim: int, num_frames: int = 5):
        super(TemporalCNNAggregator, self).__init__()
        self.num_frames = num_frames
        
        # 1D Convolutional layers for temporal modeling
    ...
    def forward(self, frame_features: torch.Tensor) -> torch.Tensor:
        ...
        """
        # Transpose for 1D convolution: (batch_size, feature_dim, num_frames)
        x = frame_features.transpose(1, 2)
        
        # Apply temporal convolutions
        x = self.conv_layers(x)  # (batch_size, feature_dim//2, 1)
        
        # Flatten and classify
        x = x.squeeze(-1)  # (batch_size, feature_dim//2)
...
class AdvancedAIDetector(nn.Module):
    ...
    def __init__(
        ...
        if temporal_head == 'temporal_cnn':
            self.aggregator = TemporalCNNAggregator(feature_dim, num_frames)
            logger.info(f"Using Temporal CNN aggregator")
        else:  # default to attention
            self.aggregator = AttentionAggregator(feature_dim, num_frames)
            logger.info(f"Using Attention aggregator")
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        features = self._extract_features(x)  # (batch_size * num_frames, feature_dim)
...
def create_advanced_model(
    ...
    Args:
        ...
        temporal_head: Type of temporal aggregation ('attention' or 'temporal_cnn')
    ...
    logger.info(f"  Temporal head: {temporal_head}")
...
def load_advanced_model(checkpoint_path: str, device: str = 'cpu') -> AdvancedAIDetector:
    ...
    # Handle older checkpoints that might not have temporal_head
    if 'temporal_head' not in model_config:
        model_config['temporal_head'] = 'attention'
