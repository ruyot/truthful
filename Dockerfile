...
COPY results/skeleton_model_v4.pt /var/models/
RUN chmod 644 /var/models/skeleton_model_v4.pt

# Copy cache script and run it to cache model weights in the image
COPY scripts/cache_weights.py /app/scripts/
ENV SKELETON_MODEL_PATH=/var/models/skeleton_model_v4.pt
...
RUN rm -f results/skeleton_model_v4.pt
